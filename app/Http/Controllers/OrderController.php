...
class OrderController extends Controller
{
    ...
    public function confirm(Request $request)
    {
        ...
        if (in_array($dataKategori->kode, $daftarGameValidasi)) {
            ...
            \Log::info('Username ditemukan controller utama: ' . ($data['data']['userNameGame'] ?? 'NULL'));

            if ($data['status']['code'] == 1 || empty($data['data']['userNameGame'])) {
                return response()->json([
                    'status' => false,
                    'data' => 'Username tidak ditemukan atau coba lagi nanti'
                ]);
            }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->payment_method == "SALDO") {
            ...
            if (!isset($channelResponse->data)) {
                $message = isset($channelResponse->message) ? $channelResponse->message : 'Gagal mengambil metode pembayaran. Silakan coba lagi nanti';
            ...
            if ($tripayres['success'] != true)
                return response()->json(['status' => false, 'data' => $tripayres['msg']]);
