...
class KategoriController extends Controller
{
    ...
    {
        return view('components.admin.kategori', ['data' => Kategori::orderBy('sort_order', 'asc')->orderBy('nama', 'asc')->get()]);
    }
    ...
    public function store(Request $request)
    {
        ...
        $kategori->is_popular = $request->is_popular ? 1 : 0;
                    ...
                    <div class='mb-3 row'>
                        ...
                        <div class='col-lg-10'>
                            <select class='form-control' name='is_popular'>
                                <option value='1' " . ($data->is_popular == 1 ? 'selected' : '') . ">Ya</option>
                                <option value='0' " . ($data->is_popular == 0 ? 'selected' : '') . ">Tidak</option>
                            </select>
                        </div>
                    </div>
                         <div class='mb-3 row'>
    ...
    public function updateSortOrder(Request $request)
    {
        try {
            $request->validate([
                ...
                'kategori_ids.*' => 'required|integer|exists:kategoris,id'
            ]);

            foreach ($request->kategori_ids as $index => $kategoriId) {
                Kategori::where('id', $kategoriId)->update(['sort_order' => $index + 1]);
            }
            ...
            \Log::error('Error updating kategori sort order: ' . $e->getMessage());

            return response()->json([
                'success' => false,
                'message' => 'Terjadi kesalahan saat memperbarui urutan kategori'
            ], 500);
        }
    }
}
